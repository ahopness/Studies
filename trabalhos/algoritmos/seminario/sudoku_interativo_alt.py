...
def on_activate(app):
    ...
    window_main.set_default_size(450, 550)
    ...
    # Label para mostrar informações de debug
    global debug_label
    debug_label = Gtk.Label()
    debug_label.set_text("Clique em 'Novo Jogo' para começar")
    box_main.append(debug_label)
    ...
    speed_box = Gtk.Box()
    box_main.append(speed_box)
    speed_box.set_spacing(12)
    speed_box.set_orientation(Gtk.Orientation.HORIZONTAL)
    speed_box.set_halign(Gtk.Align.CENTER)

    speed_label = Gtk.Label()
    speed_label.set_text("Velocidade:")
    speed_box.append(speed_label)

    global speed_scale
    # speed_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 10, 1000, 10)
    speed_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 100, 10000, 10)
    speed_scale.set_value(1000)
...
def novo_jogo(*args):
    ...
    debug_label.set_text("Novo jogo gerado! Clique em 'Resolver' para ver a animação.")
...
def resolver(*args):
            ...
            for j in range(9):
                if grid[i][j] == 0:
                    for num in range(1, 10):
                        if eh_seguro(i, j, num):
                            grid[i][j] = num
                            passos.append((i, j, num, "COLOCAR"))
                            
                            if resolver():
                                return True
                            
                            # remove o número se não funcionou
                            grid[i][j] = 0
                            passos.append((i, j, 0, "REMOVER"))
                    return False
        return True
    def eh_seguro(row, col, num):
        # linha
    ...
    def executar_passo(passo_idx):
        if passo_idx >= len(passos):
            debug_label.set_text("Sudoku resolvido com sucesso!")
            global SOLVING
            SOLVING = False
            return False
        
        i, j, valor, acao = passos[passo_idx]
        GRID_SUDOKU[i][j].get_buffer().set_text(str(valor) if valor != 0 else "", 1)
        
        # Informações de debug
        subgrid_num = (i // 3) * 3 + (j // 3) + 1
        debug_text = f"Passo {passo_idx + 1}/{len(passos)}: {acao} {valor if valor != 0 else 'vazio'} na posição ({i+1},{j+1}) - Subgrid {subgrid_num}"
        debug_label.set_text(debug_text)
        
        # Usa a velocidade do controle deslizante
        velocidade = int(speed_scale.get_value())
        GLib.timeout_add(velocidade, executar_passo, passo_idx + 1)
        return False
    
    debug_label.set_text("Calculando solução...")
    
    if resolver(): # coleta os passos e inicia a animação
        debug_label.set_text(f"Solução encontrada! {len(passos)} passos. Iniciando animação...")
        GLib.timeout_add(100, executar_passo, 0)
    else:
        debug_label.set_text("Sem solução possível")
        SOLVING = False
